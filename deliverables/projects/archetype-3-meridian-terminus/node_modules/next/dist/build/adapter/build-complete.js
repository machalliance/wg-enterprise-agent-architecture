            ...
            if (appPageKeys) {
                for (const page of appPageKeys){
                    ...
                    if (existingOutput) {
                        ...
                        await pushAsset(existingOutput.assets, existingOutput.assetsHashes, _path.default.relative(repoRoot, pageFile), pageFile, bundler);
                        continue;
                    }
...
async function getSharedNodeAssets({ dir, bundler, distDir, repoRoot, outputFileTracingRoot, requiredServerFiles, hasInstrumentationHook, config }) {
    ...
    for (const type of moduleTypes){
        ...
        for (const dependencyPath of currentDependencies){
            ...
            if (type === 'pages') {
                await pushAsset(pagesSharedNodeAssets, pagesSharedNodeAssetsHashes, rootRelativeFilePath, _path.default.join(repoRoot, rootRelativeFilePath), bundler);
            } else {
                await pushAsset(appPagesSharedNodeAssets, appPagesSharedNodeAssetsHashes, rootRelativeFilePath, _path.default.join(repoRoot, rootRelativeFilePath), bundler);
            }
        }
    }
    ...
    await pushAsset(sharedNodeAssets, sharedNodeAssetsHashes, _path.default.relative(repoRoot, setupNodeStubPath), require.resolve('next/dist/build/adapter/setup-node-env.external'), bundler);
    ...
    if (bundler !== _bundler.Bundler.Turbopack) {
        ...
        for (const tracingRootRelativeFilePath of fileList){
            ...
            await pushAsset(sharedNodeAssets, sharedNodeAssetsHashes, _path.default.relative(repoRoot, absoluteFilePath), absoluteFilePath, bundler);
        }
    }
