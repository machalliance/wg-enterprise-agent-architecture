...
async function collectPrefetchHints(fullPageDataBuffer, staleTime, clientModules, serverConsumerManifest, maxSize, maxBundleSize) {
    ...
    const headVaryParams = (0, _varyparamsdecoding.readVaryParams)(initialRSCPayload.h, rootVaryParamsIterable);
    const [, headBuffer] = await renderSegmentPrefetch(buildId, staleTime, head, _segmentvalueencoding.HEAD_REQUEST_KEY, headVaryParams, clientModules, null, // This pass only measures gzip sizes for inlining hints; fallback-ness
    ...
    const { node } = await collectPrefetchHintsImpl(flightRouterState, buildId, staleTime, seedData, clientModules, _segmentvalueencoding.ROOT_SEGMENT_REQUEST_KEY, null, maxSize, maxBundleSize, headGzipSize, headInlineState, subtreeHasRuntimePrefetch, rootVaryParamsIterable);
...
requestKey, parentGzipSize, maxSize, maxBundleSize, headGzipSize, headInlineState, routeHasRuntimePrefetch, rootVaryParamsIterable) {
    ...
    if (!isStaticPrefetchDisabled && seedData !== null) {
        ...
        const [, buffer] = await renderSegmentPrefetch(buildId, staleTime, seedData[0], requestKey, varyParams, clientModules, null, // Size-measurement pass only; fallback-ness is irrelevant here.
        false);
    ...
    for(const parallelRouteKey in children){
        ...
        didInlineIntoChild ? null : sizeToOfferChild, maxSize, maxBundleSize, headGzipSize, headInlineState, routeHasRuntimePrefetch, rootVaryParamsIterable);
        if (slots === null) {
            slots = {};
        }
...
async function PrefetchTreeData({ isClientParamParsingEnabled, fullPageDataBuffer, serverConsumerManifest, clientModules, staleTime, segmentTasks, onCompletedProcessingRouteTree, prefetchInlining, hints, isUpgradeableISRFallback }) {
    ...
    const tree = collectSegmentDataImpl(isClientParamParsingEnabled, flightRouterState, buildId, staleTime, seedData, clientModules, _segmentvalueencoding.ROOT_SEGMENT_REQUEST_KEY, segmentTasks, prefetchInlining, hints, null, headBundle, rootVaryParamsIterable, isUpgradeableISRFallback);
    // Spawn a task to produce a prefetch response for the "head" segment,
    // unless it was inlined into a page's bundle.
    if (!headIsInlined) {
        segmentTasks.push((0, _scheduler.waitAtLeastOneReactRenderTask)().then(()=>renderSegmentPrefetch(buildId, staleTime, head, _segmentvalueencoding.HEAD_REQUEST_KEY, headVaryParams, clientModules, null, isUpgradeableISRFallback)));
    }
...
function collectSegmentDataImpl(isClientParamParsingEnabled, route, buildId, staleTime, seedData, clientModules, requestKey, segmentTasks, prefetchInlining, hintTree, parentBundle, headBundle, rootVaryParamsIterable, isUpgradeableISRFallback) {
    ...
    const varyParams = (0, _varyparamsdecoding.readVaryParams)(seedData !== null ? seedData[4] : null, rootVaryParamsIterable);
    ...
    if (prefetchInlining && prefetchHints & _approutertypes.PrefetchHint.InlinedIntoChild) {
        ...
        if (seedData !== null && rsc !== null) {
            ...
            segmentTasks.push((0, _scheduler.waitAtLeastOneReactRenderTask)().then(()=>renderSegmentPrefetch(buildId, staleTime, rsc, requestKey, varyParams, clientModules, bundle, isUpgradeableISRFallback)));
        }
    ...
    for(const parallelRouteKey in children){
        ...
        const childTree = collectSegmentDataImpl(isClientParamParsingEnabled, childRoute, buildId, staleTime, childSeedData, clientModules, childRequestKey, segmentTasks, prefetchInlining, childHintTree, childBundle, headBundle, rootVaryParamsIterable, isUpgradeableISRFallback);
...
async function renderSegmentPrefetch(buildId, staleTime, rsc, requestKey, varyParams, clientModules, bundle, isUpgradeableISRFallback) {
