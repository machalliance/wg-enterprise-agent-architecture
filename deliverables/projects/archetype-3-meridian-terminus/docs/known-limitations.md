...
**1. Run state is a module-level `Map` keyed by session id.**
...
**2. The step counter can double-count a retried step.**
...
**3. `escalate_to_desk` records an escalation; it does not create anything.**
...
**4. Tier B approval never actually parks in the offline path.**
...
live agent. *Fix:* both together.

**5. Reference data is read once and cached for the process, not the run.**
...
**6. The sandbox is not isolated, by choice, and that choice is fragile to a
...
**7. Free-text remittance information reaches the model unsanitised.**
...
**8. No instruction needs two repairs where the first exposes the second.**
...
**9. AI Gateway budgets are described but not handled.**
...
**10. The trail is per-run and self-verifying, not tamper-evident against a
...
**11. `close_batch` can be called more than once.**
...
**11a. `close_batch` verifies completeness, not diligence.**
...
**12. Screening is four states in a JSON file.** No name matching, no fuzzy
...
**13. The reference data is a dozen entries per table.** A real BIC directory has
...
**14. Validation is a subset.** Real CBPR+ and SEPA validation runs to hundreds of
...
**15. No settlement, no value-date arithmetic, no cut-off times.** Cut-off
...
**16. Amounts are decimal strings.** A production system would carry integer minor
...
**17. The named parties are fiction.** Meridian Financial, Meridian Outfitters and
...
**18. There is no real pain.001 or pacs.002 on the wire.** `seed/batch.json` is a
...
**19. `ExternalStatusReason1Code` usage is plausible, not authoritative.** The
...
**20. `RR04` as the sanctions disclosure code is practice, not a rule.** There is
...
**21. The travel-rule handling rests on FATF INR.16, and the reading is a
