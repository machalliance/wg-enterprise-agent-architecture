 ...
 */ async function generateStagedDynamicFlightRenderResultNode(req, ctx, requestStore) {
    ...
    const shellByteLengthDeferred = appShells ? (0, _promisewithresolvers.createPromiseWithResolvers)() : null;
    ...
    const rscPayload = await _workunitasyncstorageexternal.workUnitAsyncStorage.run(requestStore, generateDynamicRSCPayload, ctx, {
        ...
        shellByteLengthPromise: shellByteLengthDeferred == null ? void 0 : shellByteLengthDeferred.promise,
    ...
    const flightStream = await (0, _apprenderrenderutils.runInSequentialTasks)(()=>{
        ...
        void countShellAndStaticStageBytes(staticStream, stageController).then((byteLengths)=>{
            ...
            shellByteLengthDeferred == null ? void 0 : shellByteLengthDeferred.resolve(byteLengths[_stagedrendering.RenderStage.ShellStatic]);
        });
 ...
 */ async function spawnRuntimePrefetchWithFilledCaches(writable, ctx, prerenderResumeDataCache, requestStore, onError) {
    try {
        ...
        const { result } = await finalRuntimeServerPrerender(mode, ctx, generateDynamicRSCPayload.bind(null, ctx, {
            ...
            shellUsedSessionDataPromise: mode.type !== 'runtime-only' ? mode.shellUsedSessionDataDeferred.promise : undefined
...
async function generateRuntimePrefetchResult(req, ctx, requestStore, isShellPrefetch) {
    ...
    const prerenderResumeDataCache = (0, _resumedatacache.createPrerenderResumeDataCache)();
    await prospectiveRuntimeServerPrerender(ctx, generateDynamicRSCPayload.bind(null, ctx), prerenderResumeDataCache, rootParams, requestStore.headers, requestStore.cookies, requestStore.draftMode);
    const mode = appShells ? isShellPrefetch ? {
...
async function prospectiveRuntimeServerPrerender(ctx, getPayload, resumeDataCache, rootParams, headers, cookies, draftMode) {
...
async function finalRuntimeServerPrerender(mode, ctx, getPayload, resumeDataCache, rootParams, headers, cookies, draftMode, onError, staleTimeIterable, debugChannel) {
    ...
    const stageByteLengths = mode.type === 'rewindable-session-shell' || mode.type === 'session-shell-only' ? createStageByteLengths() : null;
   ...
   * */ const checkUnexpectedAbort = ()=>{
    ...
    await (0, _apprenderrenderutils.runInSequentialTasks)(async ()=>{
        ...
        if ((mode.type === 'rewindable-session-shell' || mode.type === 'session-shell-only') && stageByteLengths) {
...
async function renderToStream(requestStore, req, res, ctx, tree, formState, postponedState, metadata, createRequestStore, fallbackParams) {
    ...
    let bootstrapScriptContent = process.env.__NEXT_DEV_SERVER ? `self.__next_r=${JSON.stringify(requestId ?? crypto.randomUUID())}` : undefined;
 ...
 */ function runDevValidationInBackground(prefetchMode, navigationKind, resultPromise, requestStore, validationDebugChannel, cacheSignal, ctx, fallbackRouteParams, prerenderResumeDataCache, getDevRenderDidError, createRequestStore, getPayload, onError, requestAbortSignal) {
    void _consoleasyncstorageexternal.consoleAsyncStorage.run({
        ...
        // below).
        if (requestAbortSignal == null ? void 0 : requestAbortSignal.aborted) {
        ...
        const lazyInputs = await prepareValidationInputs(prefetchMode, navigationKind, result, requestStore, validationDebugChannel, ctx, prerenderResumeDataCache, createRequestStore, getPayload, onError, requestAbortSignal);
        ...
        return runValidationInDev(prefetchMode, instantInputs, staticInputs, ctx, fallbackRouteParams, devRenderDidError, requestAbortSignal);
...
async function prepareValidationInputs(prefetchMode, navigationKind, result, requestStore, validationDebugChannel, ctx, prerenderResumeDataCache, createRequestStore, getPayload, onError, requestAbortSignal) {
    ...
    if (prefetchMode === 2) {
        return prepareValidationInputsInPartialPrefetching(navigationKind, requestStore, ctx, prerenderResumeDataCache, createRequestStore, getPayload, onError, inputsFromNavigation, requestAbortSignal);
    } else {
        return prepareValidationInputsInLegacyPrefetching(ctx, prerenderResumeDataCache, createRequestStore, getPayload, onError, inputsFromNavigation, requestAbortSignal);
    }
}
async function prepareValidationInputsInPartialPrefetching(navigationKind, requestStore, ctx, prerenderResumeDataCache, createRequestStore, getPayload, onError, inputsFromNavigation, requestAbortSignal) {
    const loaderTree = ctx.componentMod.routeModule.userland.loaderTree;
    ...
    const LAZY_FULL_RENDER = createLazyDevValidationInputs(async ()=>{
        ...
        const inputs = await renderWithWarmCachesForValidationInDev(ctx, createRequestStore, getPayload, onError, prerenderResumeDataCache, prefetchMode, shouldRenderWithAppShell, requestAbortSignal);
    ...
    const LAZY_RUNTIME_PRERENDER = createLazyDevValidationInputs(async ()=>{
        const inputs = await prerenderWithWarmCachesForStaticValidationInDev(ctx, createRequestStore, getPayload, onError, prerenderResumeDataCache, requestAbortSignal);
        if (forwardErrorsFromWarmRender(inputs, ctx)) {
            return VALIDATION_BAILOUT;
        }
        return inputs;
...
async function prepareValidationInputsInLegacyPrefetching(ctx, prerenderResumeDataCache, createRequestStore, getPayload, onError, inputsFromNavigation, requestAbortSignal) {
    const loaderTree = ctx.componentMod.routeModule.userland.loaderTree;
    const needsInstantValidation = await (0, _instantconfig.anySegmentNeedsInstantValidationInDev)(loaderTree);
...
async function renderWithWarmCachesForValidationInDev(ctx, createRequestStore, getPayload, onError, prerenderResumeDataCache, prefetchMode, shouldRenderWithAppShell, requestAbortSignal) {
    ...
    const accumulatedChunks = await (0, _apprenderrenderutils.runInSequentialTasks)(()=>{
        ...
        return accumulateStreamChunks(sourceStream, stageController, requestAbortSignal ?? null);
...
async function prerenderWithWarmCachesForStaticValidationInDev(ctx, createRequestStore, getPayload, onError, prerenderResumeDataCache, requestAbortSignal) {
 ...
 */ async function stagedRenderWithCachesInDev({ prefetchMode, ctx, requestStore, createRequestStore, getPayload, onError, shouldValidate, fallbackRouteParams, getDevRenderDidError, navigationKind, requestAbortSignal }) {
    ...
    if (shouldValidate) {
        runDevValidationInBackground(prefetchMode, navigationKind, resultPromise, requestStore, validationDebugChannel, cacheSignal, ctx, fallbackRouteParams, prerenderResumeDataCache, getDevRenderDidError, createRequestStore, getPayload, onError, requestAbortSignal);
    } else {
        logValidationSkipped(ctx);
 ...
 */ async function runValidationInDevImpl(prefetchMode, instantInputs, staticInputs, ctx, fallbackRouteParams, devRenderDidError, requestAbortSignal) {
    ...
    //================================
    {
        ...
        needsInstantValidation ? 'validation-client' : 'prerender-client', needsInstantValidation ? dynamicChunks : runtimeChunks, dynamicChunks, rootParams, fallbackRouteParams, ctx, validationSamples, validationSampleTracking);
    }
    ...
    //================================
    {
        ...
        const result = await validateStaticShell(inputs, ctx, rootParams, fallbackRouteParams, debugChunks, hmrRefreshHash);
    ...
    if (needsInstantValidation && instantInputs) {
        ...
        const debugChunks = inputs.debugChannelClient ? await getDebugChunksOnce(inputs.debugChannelClient) : null;
        const hmrRefreshHash = (0, _workunitasyncstorageexternal.getHmrRefreshHash)(inputs.requestStore);
        const result = await validateInstantConfigs(prefetchMode, inputs.accumulatedChunks, debugChunks, inputs.startTime, rootParams, fallbackRouteParams, ctx, hmrRefreshHash, validationSamples, devRenderDidError);
...
async function validateStaticShell(inputs, ctx, rootParams, fallbackRouteParams, debugChunks, hmrRefreshHash) {
    const debug = process.env.NEXT_PRIVATE_DEBUG_VALIDATION === '1' ? console.log : undefined;
...
async function warmupClientModulesForStagedValidation(storeType, partialServerChunks, allServerChunks, rootParams, fallbackRouteParams, ctx, validationSamples, validationSampleTracking) {
 ...
 */ async function validateInstantConfigs(prefetchMode, accumulatedChunks, debugChunks, startTime, rootParams, fallbackRouteParams, ctx, hmrRefreshHash, validationSamples, devRenderDidError) {
    const debug = process.env.NEXT_PRIVATE_DEBUG_VALIDATION === '1' ? console.log : undefined;
    ...
    async function validateAtDepthImpl(prefetchKind, depth, groupDepthForValidation, previousBoundaryState) {
        ...
        const payloadResult = await createCombinedPayloadAtDepth(prefetchKind, initialRscPayload, cache, loaderTree, ctx.getDynamicParamFromSegment, ctx.query, depth, groupDepthForValidation, extraChunksController.signal, boundaryState, clientReferenceManifest, stageEndTimes, useRuntimeStageForPartialSegments);
...
async function prerenderToStream(req, res, ctx, metadata, tree, fallbackRouteParams) {
    ...
    let bootstrapScriptContent = renderOpts.experimental.exposeTestingApi ? await (0, _instanttestbootstrap.getInstantTestBootstrapScriptContent)() : undefined;
    ...
    try {
       ...
       */ // The prerender controller represents the lifetime of the prerender. It
            ...
            const streamState = createStreamPendingState();
            const collectedChunks = createPrerenderChunksAccumulator();
            const collectedChunksByStage = appShells ? createStageChunksAccumulator() : null;
            const collectChunk = (chunk)=>{
