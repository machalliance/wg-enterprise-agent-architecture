            ...
            case 'prerender-runtime':
                {
                    ...
                    const stagedRendering = outerWorkUnitStore.stagedRendering;
                    if (stagedRendering) {
                        await stagedRendering.waitForStage(// TODO(app-shells): exclude private caches with a short staletime from shells
                        _dynamicrenderingutils.RENDER_STAGES_BY_DATA_KIND.sessionData);
                    }
                    break;
                }
            case 'request':
                {
                    if (process.env.NODE_ENV === 'development') {
                        ...
                        await (0, _dynamicrenderingutils.makeDevtoolsIOAwarePromise)(undefined, outerWorkUnitStore, // TODO(app-shells): exclude private caches with a short staletime
                        _dynamicrenderingutils.RENDER_STAGES_BY_DATA_KIND.sessionData);
                    }
                    break;
                }
    ...
    if (resumeDataCache) {
        ...
        if (rdcEntry !== undefined) {
            ...
            if (rdcResult !== undefined) {
                        ...
                        case 'prerender-runtime':
                            {
                                ...
                                if (stagedRendering) {
                                    await stagedRendering.waitForStage(// TODO(app-shells): exclude caches with a short staletime
                                    _dynamicrenderingutils.RENDER_STAGES_BY_DATA_KIND.sessionData);
                                }
                                break;
                            }
                ...
                if (rdcResult.entry.stale < _constants.MIN_PREFETCHABLE_STALE) {
                    switch(workUnitStore.type){
                        case 'prerender':
                        case 'prerender-runtime':
    ...
    if (stream === undefined) {
        ...
        try {
            ...
            while(stream === undefined){
                ...
                if (entry !== undefined && entry.stale < _constants.MIN_PREFETCHABLE_STALE) {
                    switch(workUnitStore.type){
                        case 'request':
                            {
                                // A short stale time excludes the entry from prerenders.
