 ...
 */ function assignDefaultsAndValidate(dir, userConfig, silent, phase) {
    ...
    if (result.experimental.appShells) {
        ...
        if (missing.length > 0) {
            throw Object.defineProperty(new Error(`\`experimental.appShells\` requires the following to also be enabled: ${missing.join(', ')}. Please update your ${configFileName} accordingly.`), "__NEXT_ERROR_CODE", {
                value: "E1279",
                enumerable: false,
                configurable: true
            });
 ...
 */ function finalizeConfig(config) {
    ...
    if (config.experimental.supportsImmutableAssets && (config.output === 'export' || config.output === 'standalone')) {
...
async function loadConfigImpl(phase, dir, { customConfig, rawConfig, silent = true, reportExperimentalFeatures, reactProductionProfiling, debugPrerender, bundler }) {
    ...
    if (path == null ? void 0 : path.length) {
        ...
        const finalConfig = finalizeConfig(await applyModifyConfig(completeConfig, phase, silent, dir));
...
function enforceExperimentalFeatures(config, options) {
    ...
    if (config.cacheComponents && config.experimental.varyParams !== false && config.experimental.optimisticRouting !== false && config.experimental.cachedNavigations !== false && (config.experimental.appShells === undefined || isDefaultConfig && !config.experimental.appShells)) {
